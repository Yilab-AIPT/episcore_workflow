...
PROFILE="${PROFILE:-at_analyze,alioth_slurm}"
NF_OPTS="${NF_OPTS:-}"
...
for recall in "${CPG_RECALLS[@]}"; do
    cpg_file="${SCRIPT_DIR}/assets/grid_search/CpG_recall${recall}.txt"
    ref_matrix="${SCRIPT_DIR}/assets/grid_search/ref_matrix_thres_${THRESHOLD}_cpg_${recall}.tsv"
    outdir="/lustre1/cqyi/syfan/nf_autogluon_infer/results/220k_panel/grid_search/thres_${THRESHOLD}_cpg_${recall}_analyze"
    ...
    nf_cmd=(
        ...
        --threshold "$THRESHOLD"
        --cpg_list "$cpg_file"
        --reference_beta_zscore_matrix "$ref_matrix"
        $NF_OPTS
    )
