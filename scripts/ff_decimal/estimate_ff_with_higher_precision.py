#!/usr/bin/env python3
"""
High-Precision Fetal Fraction Estimation for Aneuploidy Detection

This script is a higher-precision variant of ``bin/estimate_ff.py``. It uses
the same likelihood model implemented in ``bin/FFEstimator.py`` (Hardy-Weinberg
mixture model over maternal/fetal genotypes), but replaces the single
fixed-grid search with an iterative range-narrowing search:
...
# Make ``bin/FFEstimator.py`` and ``bin/estimate_ff.py`` importable. Both live
# in <repo>/bin and we are at <repo>/scripts/ff_decimal/.
...
def main(
    ...
    """High-precision Fetal Fraction Estimation Tool.
