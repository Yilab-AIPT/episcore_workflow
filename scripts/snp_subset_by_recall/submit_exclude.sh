#!/usr/bin/bash
# Submit build_exclude_snp_subset.py jobs for recalls 0.05 .. 0.95 (step 0.05).
#
...
SLURM_SCRIPT=run_build_exclude_snp_subset.slurm
JOB_PREFIX=snp_excl_r

mkdir -p logs

# 0.05, 0.10, ..., 0.95 with %g (0.10 -> 0.1) to match recall filenames.
mapfile -t RECALLS < <(awk 'BEGIN { for (r = 0.05; r <= 0.95 + 1e-9; r += 0.05) printf "%g\n", r }')
