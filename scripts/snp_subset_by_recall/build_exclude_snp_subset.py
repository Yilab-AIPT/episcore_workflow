#!/usr/bin/env python3
"""
Build per-recall SNP site lists from 220k CpG recall filtering (exclude overlap).

For a target CpG recall, keep 220k probe regions that do **not** overlap any CpG in
...
DEFAULT_OUT_DIR = (
    "/lustre1/cqyi/AIPT_2.0/data/meta/episcore/20260604-snp_subset/snp_exclude_subset"
)


def panel_regions_overlapping_cpgs(panel: pl.DataFrame, cpgs: pl.DataFrame) -> pl.DataFrame:
    """Probe regions overlapping at least one CpG (join_asof per chromosome)."""
    probes = (
...
def panel_regions_not_overlapping_cpgs(panel: pl.DataFrame, cpgs: pl.DataFrame) -> pl.DataFrame:
    """Keep probe regions with no overlap to any CpG in the recall list."""
    overlapping = panel_regions_overlapping_cpgs(panel, cpgs)
    return panel.join(overlapping, on=["chr", "start", "end"], how="anti")
...
def build_one_recall(
    ...
    out_path = out_dir / f"snp_exclude_for_recall_{recall_str}.tsv"
    ...
    console.print(f"[bold]Recall {recall_str} (exclude overlap)[/bold]")
    ...
    overlapping_probes = panel_regions_overlapping_cpgs(panel_220k, autosome_cpgs)
    excluded_probes = panel_regions_not_overlapping_cpgs(panel_220k, autosome_cpgs)
    filtered_snps = filter_snps_by_probes(snp_sites, excluded_probes)
    ...
    n_overlap = overlapping_probes.height
    n_probes = excluded_probes.height
    n_snps_in = snp_sites.height
    n_snps_out = filtered_snps.height

    console.print(
        f"  probes overlapping CpGs : {n_overlap:,} / {n_panel:,} "
        f"({n_overlap / n_panel:.4f})"
    )
    console.print(
        f"  probes retained (no overlap) : {n_probes:,} / {n_panel:,} "
...
@click.option("--recall-max", type=float, default=0.95, show_default=True)
...
def main(
    ...
    """Build SNP subset TSVs for probes not overlapping recall CpG lists."""
