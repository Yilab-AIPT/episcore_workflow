...
@click.option(
    '--output-prefix',
    required=True,
    help='Output prefix for the merged data. The output file will be saved as {prefix}.tsv.gz'
)
def main(target_pileup: str, background_pileup: str, output_prefix: str) -> None:
    """
    ...
    Args:
        ...
        output_prefix (str): Output prefix for the merged data.
    ...
    with Progress(
        ...
        # Write output
        progress.update(task, description=f"Writing output to {output_prefix}.tsv.gz...")
        try:
            output_path = write_output_file(merged_df, output_prefix)
